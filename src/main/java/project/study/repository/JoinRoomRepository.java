...
public class JoinRoomRepository {
    ...
    public List<ResponseRoomListDto> getRoomInfo(Member member) {
        ...
        return query.select(Projections.fields(ResponseRoomListDto.class,
                ...
                r.roomPublic.stringValue().eq(PublicEnum.PUBLIC.name()).as("roomPublic"),
                j.member.memberId.castToNum(Long.class).eq(member.getMemberId()).as("roomJoin"),
            ...
            .where(j.member.memberId.castToNum(Long.class).eq(member.getMemberId()))
            .fetch();
    }
    ...
    public List<ResponseRoomListDto> search(Long memberId, String word, Pageable pageable) {
        ...
        return query.selectDistinct(Projections.fields(ResponseRoomListDto.class,
            ...
            .where((getLowerAndReplace(r.roomTitle).likeIgnoreCase(keywordExpression)
                .or(getLowerAndReplace(r.roomIntro).likeIgnoreCase(keywordExpression))
                .or(getLowerAndReplace(r.tags.any().tagName).likeIgnoreCase(keywordExpression)))
    ...
    private BooleanExpression getRoomJoin(Long memberId, QJoinRoom j, QRoom r) {
        ...
        return JPAExpressions
            ...
            .where(r.eq(j.room).and(j.member.memberId.eq(memberId)))
            .exists();
